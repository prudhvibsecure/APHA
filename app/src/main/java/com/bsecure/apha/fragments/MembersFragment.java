...
import com.bsecure.apha.adapters.ApproveMembersAdapter;
...
public class MembersFragment extends ParentFragment implements ApproveMembersAdapter.ContactAdapterListener, JsonHandler {
    ...
    private ApproveMembersAdapter adapter;
            ...
            switch (requestType) {
                ...
                case 1:
                    ...
                    if (object.optString("statuscode").equalsIgnoreCase("200")) {
                        JSONArray jsonarray = object.getJSONArray("pending_memb_details");
                        layout.findViewById(R.id.counter).setVisibility(View.VISIBLE);
                        ((TextView)layout.findViewById(R.id.counter)).setText("Pending Members: "+object.optString("pending_count"));
                        ...
                        adapter = new ApproveMembersAdapter(vipModelArrayList, getActivity(), this);
                        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(getActivity());
                        recyclerView.setLayoutManager(linearLayoutManager);
                        recyclerView.setAdapter(adapter);
                    } else {
                        layout.findViewById(R.id.counter).setVisibility(View.GONE);
                        adapter.clear();
    ...
    public void onRowClicked(List<Members> matchesList, boolean value, Switch chk_name, int position) {
        if (chk_name.isChecked() == true) {
            ...
            try {
                JSONObject object = new JSONObject();
                object.put("memberids", matchesList.get(position).getMember_id() + "," + "1");
                HTTPNewPost task = new HTTPNewPost(getActivity(), this);
                task.userRequest("Processing...", 2, Paths.set_members_approve_status, object.toString(), 1);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
